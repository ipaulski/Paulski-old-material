...
public class MySearch_Paulski
{
    private int myArray_Repeat[];
    private int myArray_NoRepeat[];

    public MySearch_Paulski(int k){
        for(int x = 0; x < k-1; x++){
    ...
    public int getIndex_L(int[] a, int k){
        int t = 0;
        for(int x = 0; x < k-1; x++){
            if(a[x] == k){
                t = x;
    ...
    public static void main (String args[]){
        //System.out.println(getIndex_L(myArray_Repeat, 5));
        //System.out.println(getIndex_L(myArray_NoRepeat, 5));
    }
}
